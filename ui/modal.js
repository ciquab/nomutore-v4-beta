...
ご提示いただいた index.html と modal.js、index.js を詳細に分析しました。 カスタムチェックメニューが表示されない原因は、「画面（タブ）を切り替えた時に、メニューを描画する処理（renderCheckEditor）が実行されていないこと」 にあります。

また、index.html 内に「設定タブ」と「設定モーダル」のコードが重複して存在しており、これがプログラムの誤動作（IDの重複競合）を引き起こしています。

以下の手順で修正を行ってください。

1. ui/modal.js の修正
設定画面を描画する関数を openSettings から renderSettings に改名し、モーダルを開く処理（toggleModal）を削除します（設定はタブ画面になったため）。
